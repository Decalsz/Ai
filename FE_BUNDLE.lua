...
local State = {
    ...
    Settings = {
        ...
        EmoteLoop = true,
        MoveWhileEmote = true,
        -- 7yd7-style emote controls; MoveWhileEmote remains as a compatibility alias.
        EmoteWalkEnabled = true,
        EmoteSpeedEnabled = true,
        EmoteSpeed = 1,
...
local function playEmote(itemOrId, name)
    ...
    pcall(function()
        ...
        local playbackSpeed = State.Settings.EmoteSpeedEnabled and (tonumber(State.Settings.EmoteSpeed) or 1) or 1
...
-- 7yd7-compatible defaults/aliases.
if State.Settings.EmoteWalkEnabled == nil then
    State.Settings.EmoteWalkEnabled = State.Settings.MoveWhileEmote ~= false
end
if State.Settings.EmoteSpeedEnabled == nil then
    State.Settings.EmoteSpeedEnabled = true
end
State.Settings.MoveWhileEmote = State.Settings.EmoteWalkEnabled
...
function stage8InstallMovementInterrupt()
    ...
    -- 7yd7 behavior: only stop the emote when actual movement input exists.
    -- Humanoid.Running/StateChanged are intentionally not used here because
    -- they are more sensitive to bumps/collisions and were causing the Mix
    -- emote to stop unexpectedly.
    add(Connections.Movement8, RunService.Stepped:Connect(function()
        local track = State.Playback.Track
        if not track or not track.IsPlaying then return end

        local walkEnabled = State.Settings.EmoteWalkEnabled
        if walkEnabled == nil then
            walkEnabled = State.Settings.MoveWhileEmote ~= false
        end

        if not walkEnabled and hum.MoveDirection.Magnitude > 0 then
...
playEmote = function(itemOrId, name)
    ...
    if ok then
        if State.Playback.Track then
            pcall(function()
                -- 7yd7 keeps emotes at Action priority even while walking.
                State.Playback.Track.Priority = Enum.AnimationPriority.Action
            end)
        end
...
function stage8ApplyMoveMode(live)
    State.Settings.MoveWhileEmote = State.Settings.EmoteWalkEnabled ~= false
...
function stage9CycleSpeed()
    ...
    if State.Playback.Track then
        pcall(function()
            State.Playback.Track:AdjustSpeed(State.Settings.EmoteSpeedEnabled and nextVal or 1)
        end)
    end
...
function stage9ToggleEmoteWalk()
    local enabled = State.Settings.EmoteWalkEnabled
    if enabled == nil then enabled = State.Settings.MoveWhileEmote ~= false end
    State.Settings.EmoteWalkEnabled = not enabled
    State.Settings.MoveWhileEmote = State.Settings.EmoteWalkEnabled
    stage8ApplyMoveMode(true)
    saveData()
    rebuildMiniControls()
    if State.CurrentPage == "Settings" then renderSettings() end
    notify(State.Settings.EmoteWalkEnabled and "7yd7 Walk+Emote ON" or "7yd7 Walk+Emote OFF", true)
end

function stage9ToggleEmoteSpeed()
    local enabled = State.Settings.EmoteSpeedEnabled ~= false
    State.Settings.EmoteSpeedEnabled = not enabled

    if State.Playback.Track then
        pcall(function()
            State.Playback.Track:AdjustSpeed(
                State.Settings.EmoteSpeedEnabled and (tonumber(State.Settings.EmoteSpeed) or 1) or 1
            )
        end)
    end
    ...
    rebuildMiniControls()
    if State.CurrentPage == "Settings" then renderSettings() end
    notify(State.Settings.EmoteSpeedEnabled and "7yd7 Speed Emote ON" or "7yd7 Speed Emote OFF", true)
end
...
setCustomSlotFromBundle = function(stateName, bundleId, bundleName)
    ...
    task.spawn(function()
        ...
        -- Preserve the complete source mapping for this Mix slot.
        -- This is especially important for Idle, where one bundle can contain
        -- Animation1/Animation2/... variants. The Mix must remember that
        -- complete set instead of only remembering CurrentForm[stateName].
        local sourceVariants = {}
        for _, m in ipairs(picked) do
            table.insert(sourceVariants, {
                State=m.State,
                Folder=m.Folder,
                Name=m.Name,
                Id=m.Id,
                Weights=type(m.Weights)=="table" and copyTable(m.Weights) or {}
            })
        end
        State.SlotMeta[stateName] = {
            Bundle=bundleName or ("Bundle "..tostring(bundleId)),
            BundleId=normalizeId(bundleId),
            Id=picked[1].Id,
            Count=#picked,
...
rebuildMiniControls = function()
    ...
    corner(mini, 14); stroke(mini, THEME.Black, 2, 0)
    label(mini, "EMOTE CTRL", UDim2.new(0,10,0,6), UDim2.new(1,-20,0,20), 12, THEME.Text)
    button(mini, (State.Settings.EmoteWalkEnabled ~= false) and "WALK+EMOTE: ON" or "WALK+EMOTE: OFF", UDim2.new(0,10,0,32), UDim2.new(1,-20,0,30), function()
        stage9ToggleEmoteWalk()
    end, (State.Settings.EmoteWalkEnabled ~= false) and THEME.Orange or THEME.Yellow, Connections.Global)
    button(mini, (State.Settings.EmoteSpeedEnabled ~= false) and "SPEED: ON" or "SPEED: OFF", UDim2.new(0,10,0,68), UDim2.new(0,72,0,28), function()
        stage9ToggleEmoteSpeed()
    end, (State.Settings.EmoteSpeedEnabled ~= false) and THEME.Orange or THEME.Yellow, Connections.Global)
    button(mini, "CYCLE " .. tostring(State.Settings.EmoteSpeed) .. "x", UDim2.new(0,88,0,68), UDim2.new(0,42,0,28), function()
        stage9CycleSpeed()
    end, THEME.Orange, Connections.Global)
                ...
                for _, m in ipairs(State.CurrentMappings) do
                    if m.State == s then
                        table.insert(variants, {
                            State=m.State, Folder=m.Folder, Name=m.Name, Id=m.Id,
                            Weights=type(m.Weights)=="table" and copyTable(m.Weights) or {}
                        })
                    end
                end
                State.SlotMeta[s].Variants = variants
                State.SlotMeta[s].Count = #variants
            end
        end
        State.LastAppliedName = bundleName or ("Bundle "..bundleId)
        saveData()
...
function S14.syncMappingsToCurrentForm()
    ...
    for _, state in ipairs(STATES) do
        ...
        if wanted ~= "" then
            local meta = State.SlotMeta and State.SlotMeta[state]

            -- MIX FIX: if this slot came from a bundle, use the exact mapping
            -- snapshot captured when the slot was selected. This preserves all
            -- Idle variants even when the other Mix slots came from other
            -- bundles.
            local variants = type(meta) == "table" and meta.Variants
            local usedSnapshot = false

            if type(variants) == "table" and #variants > 0 then
                local exact = false
                for _, m in ipairs(variants) do
                    if normalizeId(m.Id) == wanted then exact = true break end
                end
                ...
                if exact then
                    for _, m in ipairs(variants) do
                        table.insert(result, {
                            ...
                            Folder=m.Folder or S14.folderForState(state),
                            Name=m.Name or S14.nameForState(state, 1),
                            Id=normalizeId(m.Id),
                            Weights=type(m.Weights)=="table" and copyTable(m.Weights) or {}
                        })
                    end
            ...
            if not usedSnapshot then
                ...
                if exact and #matches > 0 then
                    ...
                    table.insert(result, {
                        State=state,
                        Folder=S14.folderForState(state),
                        Name=S14.nameForState(state, 1),
                        Id=wanted,
                        Weights={}
                    })
                end
...
renderSettings = function()
    ...
    y=y+72
    label(sc, "7yd7-style Emote Controls", UDim2.new(0,12,0,y), UDim2.new(1,-24,0,24), 16, THEME.Text); y=y+34
    button(sc, (State.Settings.EmoteWalkEnabled ~= false) and "Walk+Emote: ON" or "Walk+Emote: OFF", UDim2.new(0,12,0,y), UDim2.new(0,170,0,32), function()
        stage9ToggleEmoteWalk()
    end, (State.Settings.EmoteWalkEnabled ~= false) and THEME.Orange or THEME.Yellow)
    button(sc, (State.Settings.EmoteSpeedEnabled ~= false) and "Speed Emote: ON" or "Speed Emote: OFF", UDim2.new(0,194,0,y), UDim2.new(0,170,0,32), function()
        stage9ToggleEmoteSpeed()
    end, (State.Settings.EmoteSpeedEnabled ~= false) and THEME.Orange or THEME.Yellow)
    y=y+42
    local sbox = textBox(sc, "Emote speed", UDim2.new(0,12,0,y), UDim2.new(0,160,0,32), tostring(State.Settings.EmoteSpeed))
    button(sc, "SET SPEED", UDim2.new(0,184,0,y), UDim2.new(0,110,0,32), function()
        local n=tonumber(sbox.Text)
        if not n then notify("Speed harus angka", false); return end
        State.Settings.EmoteSpeed=math.clamp(n,0,10000)
        State.Settings.EmoteSpeedEnabled=true
        if State.Playback.Track then pcall(function() State.Playback.Track:AdjustSpeed(State.Settings.EmoteSpeed) end) end
        saveData(); updateRuntimeBar(); renderSettings(); notify("Speed set "..tostring(State.Settings.EmoteSpeed), true)
    end, THEME.Orange)
    y=y+42
    label(sc, "Walk+Emote sekarang mengikuti pola 7yd7: emote hanya dihentikan saat MoveDirection benar-benar bergerak. Senggolan/collision tidak lagi memakai Running/StateChanged sebagai pemicu stop.", UDim2.new(0,12,0,y), UDim2.new(1,-24,0,52), 12, THEME.Muted)
    sc.CanvasSize=UDim2.new(0,0,0,y+72)
end
